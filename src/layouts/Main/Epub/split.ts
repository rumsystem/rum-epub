...
import trxApi from 'apis/trx';
import sleep from 'utils/sleep';

export interface ZipResult { zip: ZipFile, entries: Array<Entry> }

export const verifyEpub = async (filepath: string) => {
...
export type VerifiedEpub = Awaited<ReturnType<typeof verifyEpub>>;

export const getAllTrxs = async (groupId: string) => {
  const arr = [] as Array<any>;
  for (;;) {
    const res = await contentApi.fetchContents(
      groupId,
      {
        num: 100,
...
export const getAllEpubs = async (groupId: string) => {
  const arr = await getAllTrxs(groupId);
...
export const checkTrx = async (trxId: string, groupId: string) => {
  for (;;) {
    try {
      await trxApi.fetchTrx(groupId, trxId);
      break;
    } catch (e) {
      await sleep(5000);
    }
