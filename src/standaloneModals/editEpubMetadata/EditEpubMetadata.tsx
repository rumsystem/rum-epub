...
import { dialogService, bookService, tooltipService } from '~/service';
...
export const EditEpubMetadata = observer((props: Props & { destroy: () => unknown }) => {
  ...
  const handleSubmit = async () => {
    ...
    await runLoading(
      ...
      async () => {
        await pipe(
          TE.tryCatch(
            () => bookService.upload.bookMetadata(props.groupId, props.bookId, state.form),
            identity,
          ),
          TE.matchW(
            () => {
  ...
  const loadBookMetadata = () => {
    const book = bookService.state.groupMap.get(props.groupId)?.find((v) => v.book.id === props.bookId);
    const metadata = book?.metadata?.metadata;
    runInAction(() => {
      const createForm = () => ({
        description: '',
        subTitle: '',
